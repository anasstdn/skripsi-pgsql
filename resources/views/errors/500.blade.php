                ...
                <div class="hero">
                    <div class="hero-inner text-center">
                        <div class="content content-full bg-white overflow-hidden">
                            <div class="py-4">
                                <!-- Error Header -->
                                <h1 class="display-1 text-modern invisible animated zoomInDown" data-toggle="appear">500</h1>
                                <h2 class="h3 font-w300 text-muted invisible mb-5 animated fadeInUp" data-toggle="appear">Terjadi galat pada server...</h2>
                                <!-- END Error Header -->

                                <!-- Search Form -->
                                @php
                                $data = \DB::table('data_log')->orderBy('id','DESC')->first();
                                @endphp
                                <textarea class="form-control" rows="6">Message : {{ isset($data) && !empty($data->message)?$data->message:null }}&#13;&#10;Detail : {{ isset($data) && !empty($data->formatted)?$data->formatted:null }}</textarea>
                                <!-- END Search Form -->
                            </div>
                        </div>
                        <div class="content content-full font-size-sm text-muted">
                            <!-- Error Footer -->
                            <a class="link-fx" href="{{url('/')}}">Kembali ke Beranda</a>
                            <!-- END Error Footer -->
                        </div>
                    </div>
                </div>
