								...
								<div class="row justify-content-center py-sm-3 py-md-5">
									<div class="col-sm-10 col-md-8">
										<div class="form-group">
											<label for="block-form1-name">Kategori Transaksi</label>
											<input type="text" class="form-control form-control-alt form-control-sm" id="nama_kategori" name="nama_kategori">
										</div>
										<br/>
										<div class="form-group">
...
<script type="text/javascript">
	$(function() {
		...
		$("#tambah").click(function(){
			...
			$("#judul").html('Tambah Kategori Transaksi');
            $('#mode').val('add');
            $('#form').prop('action', '{{url('kategori-transaksi/store')}}');
		});
		...
		$('#tabel').dataTable({
			...
			serverSide : true,
			ajax : {
				url:"{{ url('kategori-transaksi/get-data') }}",
				data: function (d) {
...
<script type="text/javascript">
   function ubah_data(id){
    $.ajax({
        url : "{{url('kategori-transaksi/edit/')}}/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            $("#table_width").attr('class', "col-lg-6");
            $("#createNewItemDiv").toggleClass('hidden');
            $("#judul").html('Edit Kategori Transaksi');
            $('#mode').val('edit');
            $('#form').prop('action', '{{url('kategori-transaksi/update')}}');
