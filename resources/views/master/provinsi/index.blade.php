								...
								<div class="row justify-content-center py-sm-3 py-md-5">
									<div class="col-sm-10 col-md-8">
										<div class="form-group">
											<label for="block-form1-name">Kode Provinsi</label>
											<input type="text" class="form-control form-control-alt form-control-sm" id="kode_provinsi" name="kode_provinsi">
										</div>
										<div class="form-group">
											<label for="block-form1-username">Nama Provinsi</label>
											<input type="text" class="form-control form-control-alt form-control-sm" id="nama_provinsi" name="nama_provinsi">
										</div>
										<br/>
										<div class="form-group">
...
<script type="text/javascript">
	$(function() {
		...
		$("#tambah").click(function(){
			...
			$("#judul").html('Tambah Provinsi');
            $('#mode').val('add');
            $('#form').prop('action', '{{url('provinsi/store')}}');
   ...
   function ubah_data(id){
    $.ajax({
        ...
        success: function(data)
        {
            $("#table_width").attr('class', "col-lg-6");
            $("#createNewItemDiv").toggleClass('hidden');
            $("#judul").html('Edit Provinsi');
            $('#mode').val('edit');
            $('#form').prop('action', '{{url('provinsi/update')}}');
            $('#kode_provinsi').val(data.data.kode_provinsi);
