								...
								<div class="row justify-content-center py-sm-3 py-md-5">
									<div class="col-sm-10 col-md-8">
										<div class="form-group">
											<label for="block-form1-name">Jenis Kelamin</label>
											<input type="text" class="form-control form-control-alt form-control-sm" id="jenis_kelamin" name="jenis_kelamin">
										</div>
										<br/>
										<div class="form-group">
...
<script type="text/javascript">
	$(function() {
		...
		$("#tambah").click(function(){
			...
			$("#judul").html('Tambah Jenis Kelamin');
            $('#mode').val('add');
            $('#form').prop('action', '{{url('jenis-kelamin/store')}}');
		});
		...
		$('#tabel').dataTable({
			...
			serverSide : true,
			ajax : {
				url:"{{ url('jenis-kelamin/get-data') }}",
				data: function (d) {
   ...
   function ubah_data(id){
    $.ajax({
        ...
        success: function(data)
        {
            $("#table_width").attr('class', "col-lg-6");
            $("#createNewItemDiv").toggleClass('hidden');
            $("#judul").html('Edit Jenis Kelamin');
            $('#mode').val('edit');
            $('#form').prop('action', '{{url('jenis-kelamin/update')}}');
