								...
								<div class="row justify-content-center py-sm-3 py-md-5">
									<div class="col-sm-10 col-md-8">
										<div class="form-group">
											<label for="block-form1-name">Departement</label>
											<input type="text" class="form-control form-control-alt form-control-sm" id="nama_departement" name="nama_departement">
										</div>
										<br/>
										<div class="form-group">
...
<script type="text/javascript">
	$(function() {
		...
		$("#tambah").click(function(){
			...
			$("#judul").html('Tambah Departement');
            $('#mode').val('add');
            $('#form').prop('action', '{{url('departement/store')}}');
		});
   ...
   function ubah_data(id){
    $.ajax({
        ...
        success: function(data)
        {
            $("#table_width").attr('class', "col-lg-6");
            $("#createNewItemDiv").toggleClass('hidden');
            $("#judul").html('Edit Departement');
            $('#mode').val('edit');
            $('#form').prop('action', '{{url('departement/update')}}');
            $('#nama_departement').val(data.data.nama_departement);
            $('#id').val(id);
        },
